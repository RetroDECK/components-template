...
case "$pkg_mgr" in
  apt)
    ...
    ;;
  pacman)
    sudo pacman -Syu --noconfirm flatpak flatpak-builder p7zip xmlstarlet bzip2 curl jq
    ;;
  rpm-ostree)
    ...
    ;;
  dnf)
    sudo dnf install -y flatpak flatpak-builder p7zip p7zip-plugins xmlstarlet bzip2 curl jq
    ;;
  *)
